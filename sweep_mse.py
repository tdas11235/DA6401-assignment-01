...
def plot_confusion(y_true, y_pred):
    cm = confusion_matrix(y_true, y_pred, labels=range(len(F_MNIST_CLASSES)))
    ...
    fig = ff.create_annotated_heatmap(
        ...
        annotation_text=[[f"{cm_normalized[i, j]:.1%}"
                          for j in range(len(F_MNIST_CLASSES))]
                         for i in range(len(F_MNIST_CLASSES))],
        colorscale=[
            [0.0, "red"],
            [0.4, "yellow"],
            [1.0, "green"]
